...
services:
  ...
  matrix-conduit-well-known:
    ...
    labels:
      ...
      traefik.http.routers.matrix-well-knows-insecure.rule: Host(`${APP_DOMAIN}`)
      ...
      traefik.http.routers.matrix-well-knows-local.rule: Host(`matrix-well-knows.${LOCAL_DOMAIN}`)
        && PathPrefix(`/.well-known/matrix`)
